...
# The number of books is the amount of moments divided by number of images  plus one since there is no book zero.
    ...
    # If the moment is evenly divisble by the number of books, make a new book.
    if 
        [ $(( j % $IMAGES )) -eq 0 ]; then
            mkdir $BOOK
    fi

    # Capture a moment at the time of a present from the ice melting and place it in its book, named after the timestamp of its moment.
